...
//
// VIVE EN `nucleo/` desde el 18/9 porque el motor —que ahora comparten la web
// y la app nativa— pinta las estrellas con ella. Lo que la aplica a la web
// como variables CSS (`aplicarTema`) quedó en `src/lib/paletas.ts`: es lo
// único de acá que tocaba el DOM.
...
export const PALETAS_RANGO: Record<number, Paleta> = {
  ...
  // `compartido/motor/cuerpos.ts`: violeta, azul, lila). El polvo espacial de verdad no es
