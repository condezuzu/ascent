...
async function correr(app) {
  ...
  let roto = false;
  // LA CAJA NEGRA, DESCARTADA ANTES DE CADA PASO (26/9). En la vista web con
  // el flag de diagnostico prendido, la caja negra (`Raiz`) se auto-abre con
  // cada `console.warn` web-only —useNativeDriver, pointerEvents, healthkit— y
  // TAPA la pantalla, interceptando los clics del barrido. En el telefono esos
  // warnings no existen y no se abre; es puro artefacto de la vista web. Se
  // descarta al empezar cada paso porque puede reaparecer con el warning
  // siguiente. Solo en movil: en web no hay caja negra.
  async function descartarCajaNegra() {
    if (app !== 'movil') return;
    const seguir = page.getByText('Seguir igual', { exact: true }).last();
  ...
  if (app === 'movil') {
    await paso('la subida de rango, con su coreografia', async () => {
      ...
      // web (:8090) corre SIN el flag —a proposito: con el, la caja negra tapa
      // todo por un error de three.js solo-web—, asi que aca el panel no esta y
      // esta previsualizacion no se puede disparar. NO es un fallo: la subida
      // de rango se prueba en el lado WEB (12/12) y su logica en test:db. Se
      // salta con nota en vez de fingir un timeout.
      const ajustes = page.locator('[data-testid="carril-ajustes"]');
      const diag = ajustes.getByText('Diagnóstico', { exact: true }).last();
      if (!(await diag.isVisible().catch(() => false))) {
        nota('subida de rango: omitida en la vista web (panel gateado); cubierta por el lado web y test:db');
        return;
      }
      await diag.click({ timeout: 20000 });
      const boton = ajustes.getByText(/Ver la subida de rango/i).first();
